...
def buscar_negocios(nombre, ubicacion, max_resultados=30):
    resultados = []
    ids_vistos = set()
    ...
    while places_result and len(resultados) < max_resultados:
        for lugar in places_result.get("results", []):
            ...
            detalles = gmaps.place(place_id=lugar_id).get("result", {})
...
if __name__ == "__main__":
    terminos = ["inmobiliaria", "agencia inmobiliaria", "bienes raíces", "venta casas"]
    nombre_busqueda = random.choice(terminos)
    ubicacion_busqueda = "CDMX"
    max_resultados = 50  
    # Cambia este valor (recomendado: máx 60)
